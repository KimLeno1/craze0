  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 border-b h-16 md:h-20 px-4 md:px-10 flex items-center justify-between transition-colors duration-500 ${
      ...
      <div className="flex items-center gap-3 md:gap-6">
        ...
        >
          <div className={`w-4 md:w-5 h-0.5 transition-colors ${theme === 'dark' ? 'bg-white group-hover:bg-[#00D1FF]' : 'bg-black group-hover:bg-[#00D1FF]'}`}></div>
          <div className={`w-2.5 md:w-3 h-0.5 self-start ml-2 md:ml-2.5 transition-colors ${theme === 'dark' ? 'bg-white group-hover:bg-[#00D1FF]' : 'bg-black group-hover:bg-[#00D1FF]'}`}></div>
        </button>
        ...
        <div onClick={() => onNavigateView(ViewState.LOBBY)} className="flex flex-col cursor-pointer group">
          <div className={`text-lg md:text-xl font-serif italic tracking-tighter leading-none group-hover:glow-text transition-all ${theme === 'dark' ? 'text-white' : 'text-black'}`}>
            CLOSET<span className="text-[#00D1FF] font-sans font-black not-italic ml-0.5">KRAZE</span>
          </div>
      ...
      <div className="flex items-center gap-2 md:gap-4">
        ...
        >
          ...
          {notificationCount > 0 && (
            <span className="absolute -top-1 -right-1 w-3.5 h-3.5 md:w-4 md:h-4 bg-[#00D1FF] rounded-full text-[7px] md:text-[8px] font-black flex items-center justify-center text-white border border-black animate-pulse">{notificationCount}</span>
          )}
        </button>
        ...
        >
          ...
          {cartCount > 0 && (
            <span className="absolute -top-1 -right-1 w-3.5 h-3.5 md:w-4 md:h-4 bg-[#00D1FF] rounded-full text-[7px] md:text-[8px] font-black flex items-center justify-center text-white border border-black animate-pulse">{cartCount}</span>
          )}
        </button>
      </div>
    </nav>
