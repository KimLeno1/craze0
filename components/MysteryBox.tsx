...
const MysteryBox: React.FC<MysteryBoxProps> = ({ onReveal, cost }) => {
  ...
  return (
    <div className="max-w-md mx-auto relative group">
      ...
      <div className="absolute -inset-1 bg-gradient-to-r from-[#1a73e8] to-purple-600 rounded-[32px] blur opacity-25 group-hover:opacity-50 transition duration-1000"></div>
      ...
      <div className="relative p-10 rounded-[28px] bg-zinc-950 border border-white/10 flex flex-col items-center justify-center text-center overflow-hidden">
        ...
        <div className="absolute top-4 right-6 flex gap-1">
          <div className={`w-1 h-1 rounded-full bg-[#1a73e8] ${isOpening ? 'animate-ping' : ''}`}></div>
        ...
        {!reward ? (
          <>
            <div className={`relative mb-8 transition-all duration-500 ${isOpening ? 'scale-125 rotate-6' : 'group-hover:scale-110'}`}>
              <div className="text-8xl filter drop-shadow-[0_0_15px_rgba(26,115,232,0.4)]">
                {isOpening ? '⚡' : '🔒'}
              </div>
              {isOpening && (
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-20 h-20 border-4 border-[#1a73e8]/20 border-t-[#1a73e8] rounded-full animate-spin"></div>
                </div>
              )}
            </div>
            ...
            <button
              disabled={isOpening}
              onClick={handleOpen}
              className="group relative h-14 w-full bg-white text-black rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all duration-500 hover:bg-[#1a73e8] hover:text-white disabled:opacity-50 overflow-hidden shadow-2xl"
            >
          ...
          <div className="animate-in zoom-in-95 duration-700 flex flex-col items-center">
            <div className="w-20 h-20 bg-[#1a73e8]/10 rounded-full flex items-center justify-center text-4xl mb-6 border border-[#1a73e8]/20 shadow-[0_0_30px_rgba(26,115,232,0.2)]">
              ✨
            </div>
            <div className="text-[10px] font-black text-[#1a73e8] uppercase tracking-[0.3em] mb-2">Vault Result</div>
